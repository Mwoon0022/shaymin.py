...
help = f""" ឵឵  ឵឵  ឵឵  
        `~sets <gen[no.]> <pokemon[name]>`
        gives you information about a pokemon in a certain gen and format

        `~user <username>`
        gets information about a showdown users ladder

        `~help`
        youre looking at it

        
        [support server](https://discord.gg/fy9RRcxERD)・[invite link](
  https://discord.com/api/oauth2/authorize?client_id=863733591821385748&permissions=137439333440&scope=bot)
        """

class text:

  def wave(n = 1): return '‿︵'*int(n)
  def div1(n = 1): return '<:divider:887614654203953162>'*int(n)
  def div2(n = 1): return '<:div:873583970653175828>'*int(n)


facts = [ 'Fun fact: Full form of ping is Packet InterNet Groper! Try ~ping', 'Pro tip: Join the koffing [support server](https://discord.gg/fy9RRcxERD) cuz wynaut! ', 'Also try myuu!', '~~Fun~~ fact: Pikachu is bad for pvp :c', 'e.', 'Fun fact: Ash is voiced by girls!', 'Fun fact: Extreme evoboost is the best status move!', 'Fun fact: tyranitar tackle is stronger than pikachu catastropika!', 'Fun fact: Eternatus eternamax was so powerful that its banned from all showdown formats except custom!', '~~palkia is pog~~', 'Fun fact: Purrugly is faster than lati twins', 'Fun fact: Diglet is faster than the legendary suicune known to be fast enough to walk on water!', 'Fun fact: gastly can learn ice punch and fire punch!', 'Disturbing fact: pikachu can learn fly-', 'Fun fact: Trapinch has higher attack than poliwrath!', 'Fun fact: Sound proof mons arent deaf!', 'Tip: Dm pokemon fun facts to Mwoon#6559 to get them added here!', 'E', 'I (also) need more context! ' ]

class koffing:
  name = 'Koffing'
  version = '0.0.1'

commands = [["sets","sets <gen[no.]> <pokemon[name]>","grabs some sets"]]
formats = {
  "1v1": "1v1",
  ...
  "almostanyabilitysuspecttest": "AAA (Suspect Test)",
